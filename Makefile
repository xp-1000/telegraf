...
windows:
	docker run --rm -ti -v "$(CURDIR):C:\src" -v "$(CURDIR)\output:C:\output" golang:1.9.7-windowsservercore-ltsc2016 powershell C:\src\scripts\build_sfx.ps1

linux:
	docker run --rm -ti -v "$(CURDIR):/src" -v "$(CURDIR)/output:/output" golang:1.9.7 bash /src/scripts/build_sfx.sh
